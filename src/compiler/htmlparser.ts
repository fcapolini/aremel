...
export default class HtmlParser {
	...
	parseDoc(s:string, fname?:string): HtmlDocument {
		...
		var i = this._parseNodes(ret, s, 0, origin);
		if (i < s.length) {
		...
		while ((i2 = s.indexOf('<', i1)) >= 0) {
			...
			if ((i2 = this._skipName(s, i1)) > i1) {
				...
				if (closure) {
					...
					i1 = this._parseElement(p, s, i1, i2, origin);
				}
				i3 = i1;
			} else if (!closure && (i2 = this._skipComment(s, i1, origin)) > i1) {
				if (i4 > i3) {
					new HtmlText(p.ownerDocument, p, s.substring(i3, i4),
								i3, i4, origin, false);
				}
	...
	_parseElement(p:HtmlElement, s:string,
		...
		if (!selfclose && !VOID_ELEMENTS.has(e.tagName)) {
			if (SKIP_CONTENT_TAGS.has(e.tagName)) {
				var res = this._skipContent(e.tagName, s, i1, origin);
				if (!res) {
					throw new HtmlException(
						`Unterminated tag ${e.tagName}`,
						this.origins[origin], i1, s
				...
				i1 = this._parseNodes(e, s, i1, origin);
			}
		}
		return i1;
	}

	_parseAttributes(e:HtmlElement, s:string, i2:number, origin:number) {
		var i1 = this._skipBlanks(s, i2);
		while ((i2 = this._skipName(s, i1, true)) > i1) {
			//TODO: ignore `::<attributes>`, used for comments
			var name = s.substring(i1, i2);
			var a = e.setAttribute(name, '', undefined, i1, i2, origin);
			i1 = this._skipBlanks(s, i2);
			if (s.charCodeAt(i1) === '='.charCodeAt(0)) {
				i1 = this._skipBlanks(s, i1 + 1);
				var quote = s.charCodeAt(i1);
				if (a && (quote === '"'.charCodeAt(0) || quote === "'".charCodeAt(0))) {
					i1 = this._parseValue(a, s, i1 + 1, quote, String.fromCharCode(quote), origin);
			// #if (HTML_EXTENSIONS)
				} else if (a && (quote === '['.charCodeAt(0) && s.charCodeAt(i1 + 1) === '['.charCodeAt(0))) {
					i1 = this._parseValue(a, s, i1 + 2, quote, ']]', origin);
			// #end
				} else {
	...
	_parseValue(a:HtmlAttribute, s:string, i1:number,
	...
	_skipComment(s:string, i1:number, origin:number) {
		if (s.charCodeAt(i1) === '!'.charCodeAt(0)
	...
	_skipContent(tag:string, s:string, i1:number, origin:number) {
		var i2;
		while ((i2 = s.indexOf('</', i1)) >= 0) {
			var i0 = i2;
			i1 = i2 + 2;
			i2 = this._skipName(s, i1);
			if (i2 > i1) {
				if (s.substring(i1, i2).toUpperCase() === tag) {
					i2 = this._skipBlanks(s, i2);
					if (s.charCodeAt(i2) != '>'.charCodeAt(0)) {
						throw new HtmlException(
	...
	_skipName(s:string, i:number, acceptsDots=false) {
		while (i < s.length) {
